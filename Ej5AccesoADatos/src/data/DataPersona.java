...
public class DataPersona {
	...
	private void setPersona(Persona p, DataRol dr, ResultSet rs) {
		try {
	...
	public LinkedList<Persona> getByApellido(String apellido){
		...
		DataRol dr=new DataRol();
		try{
			statement = DbConnector.getInstancia().getConn().prepareStatement("SELECT * FROM persona WHERE apellido = ?");
			statement.setString(1,apellido);
			rs = statement.executeQuery();
			while(rs.next()){
				Persona p=new Persona();
				this.setPersona(p,dr,rs);
				personas.add(p);
			}
